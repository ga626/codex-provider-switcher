...
![服务商工作区：使用离线示例服务商管理名称、接口地址、可搜索的默认模型和本机保存的访问密钥状态；右侧固定显示测试与切换信息。](docs/assets/readme/provider-workspace-v0.10.10.png)

> 本页截图对应 v0.10.10-alpha 的离线示例配置：不包含真实服务商、地址、密钥、本机路径或 Codex 内容，也不会连接任何服务商。实际界面会显示运行版本、渠道和构建标识。
...
![首次启动页面：用户先阅读将要保护的范围，选择要管理的 Codex 配置层并确认创建恢复点。](docs/assets/readme/first-run-v0.10.10.png)

![使用说明目录：初始化配置必经一次；之后可随时按服务商、安全与恢复、活动记录、实验室或整体功能重新查看。](docs/assets/readme/guide-hub-v0.10.10.png)

首次初始化完成后不会因普通升级再次强制执行。状态栏或各工作区标题旁的问号可打开“使用说明”目录，按需要进入对应章节。

![安全与恢复工作区：显示首次启动基线、连接环境与恢复前必须输入“恢复”的二次确认。](docs/assets/readme/configuration-protection-v0.10.10.png)
...
![活动记录工作区：以不含密钥或配置正文的条目记录保存、检查、切换与恢复操作。](docs/assets/readme/activity-v0.10.10.png)
...
![性价比中心：脱敏示例费用记录会显示同一模型的人民币成本、官方成本占比和排名；运行固定测试后，可自动读取可用的费用候选值，或提示用户从平台日志复制。](docs/assets/readme/cost-center-v0.10.10.png)

![应用设置：检查更新会明确显示检查时间、下载进度或可操作的网络/代理错误；普通升级不会重复触发首次准备或初始化说明。](docs/assets/readme/application-settings-v0.10.10.png)

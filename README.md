...
![服务商工作区：使用离线示例服务商管理名称、接口地址、可搜索的默认模型和本机保存的访问密钥状态；右侧固定显示测试与切换信息。](docs/assets/readme/provider-workspace-v0.10.7.png)

> 本页截图对应 v0.10.7-alpha 的离线示例配置：不包含真实服务商、地址、密钥、本机路径或 Codex 内容，也不会连接任何服务商。实际界面会显示运行版本、渠道和构建标识。
...
2. **添加服务商。** 进入工作台后，点击左侧加号，填写名称、接口地址和访问密钥。密钥默认只显示星号，眼睛按钮只在本机临时显隐。
3. **保存并选择模型。** 保存服务商后，刷新模型目录；默认模型输入框支持展开、滚动选择和模糊搜索，例如输入 `5.6`。
...
![首次启动页面：用户先阅读将要保护的范围，选择要管理的 Codex 配置层并确认创建恢复点。](docs/assets/readme/first-run-v0.10.7.png)

![安全与恢复工作区：显示首次启动基线、连接环境与恢复前必须输入“恢复”的二次确认。](docs/assets/readme/configuration-protection-v0.10.7.png)
...
![活动记录工作区：以不含密钥或配置正文的条目记录保存、检查、切换与恢复操作。](docs/assets/readme/activity-v0.10.7.png)
...
![性价比中心：脱敏示例费用记录会显示同一模型的人民币成本、官方成本占比和排名；运行固定测试后，可自动读取可用的费用候选值，或提示用户从平台日志复制。](docs/assets/readme/cost-center-v0.10.7.png)

...
“实验室 > 性价比中心”用于比较同一模型在不同服务商上的实际成本。先运行一条极短的固定测试：如果服务商响应中返回本次费用，应用会自动填入“测试额度”；没有返回时，按提示从该服务商的使用日志复制即可。再填写“充值金额”和“平台实际额度”，即可得到这次固定测试的人民币成本和同模型排名。

它不读取你的账单、不自动充值，也不会因为测试而切换服务商或改写 Codex 配置。订阅制同样可以使用：把订阅价格和平台给出的固定额度填入对应字段；不同模型、不同测试记录不应混在同一排名中。

![性价比中心：脱敏示例费用记录会显示同一模型的人民币成本与排名；运行固定测试后，可自动读取可用的费用候选值，或提示用户从平台日志复制。](docs/assets/readme/cost-center-v0.10.png)

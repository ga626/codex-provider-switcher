...
![服务商工作区：使用离线示例服务商管理名称、接口地址、可搜索的默认模型和本机保存的访问密钥状态；右侧固定显示测试与切换信息。](docs/assets/readme/provider-workspace-v0.10.11.png)

> 本页截图对应 v0.10.11-alpha 的离线示例配置：不包含真实服务商、地址、密钥、本机路径或 Codex 内容，也不会连接任何服务商。实际界面会显示运行版本、渠道和构建标识。
...
2. **按初始化说明完成第一次连接。** 环境准备完成后会自动打开八步说明：新增服务商、填写名称/接口/密钥、选择模型、保存、运行测试、检查并切换。可以随时选择“稍后再说”。
3. **填写并选择模型。** 填好名称、接口和访问密钥后，默认模型框可以直接刷新模型目录，无需先保存；支持展开、滚动选择和模糊搜索，例如输入 `5.6`。刷新和选择只更新当前表单，最后仍需点击“保存更改”。
4. **运行可用性测试。** 右侧固定栏会用当前服务商执行一次最小真实请求，并分别显示超时、限流、认证、响应形态和目录结果。
5. **查看检查结果后确认切换。** 检查并切换会再次核对写入安全；完成后关闭当前 Codex 会话并重新打开，再确认实际工作。需要回退时，到“安全与恢复”选择恢复点。

![首次启动页面：用户先阅读将要保护的范围，选择要管理的 Codex 配置层并确认创建恢复点。](docs/assets/readme/first-run-v0.10.11.png)

![使用说明目录：初始化配置必经一次；之后可随时按服务商、安全与恢复、活动记录、实验室或整体功能重新查看。](docs/assets/readme/guide-hub-v0.10.11.png)

首次初始化完成后不会因普通升级再次强制执行。状态栏或各工作区标题旁的问号可打开“使用说明”目录，按需要进入对应章节。

![安全与恢复工作区：显示首次启动基线、连接环境与恢复前必须输入“恢复”的二次确认。](docs/assets/readme/configuration-protection-v0.10.11.png)
...
![活动记录工作区：默认按时间显示全部操作；需要处理的检查会按原因合并，并可查看与复制完整脱敏诊断包。](docs/assets/readme/activity-v0.10.11.png)
...
![性价比中心：脱敏示例费用记录会显示同一模型的人民币成本、官方成本占比和排名；运行固定测试后，可自动读取可用的费用候选值，或提示用户从平台日志复制。](docs/assets/readme/cost-center-v0.10.11.png)

![应用设置：检查更新会明确显示检查时间、下载进度或可操作的网络/代理错误；普通升级不会重复触发首次准备或初始化说明。](docs/assets/readme/application-settings-v0.10.11.png)

...
export function LabWorkspace({ state, selectedProfile, busy, runAction, onRunCostTest, onOpenGuide }: {
  ...
  const ranking = Array.from(new Map(state.profiles.map((item) => [item.id, item])).values()).flatMap((provider) => {
    ...
    return [{ provider, samples, median, effectiveRate: effectiveRates.length > 0 ? medianScaled(effectiveRates) : null, creditUnitLabel: unitLabels.size === 1 ? samples[0]?.creditUnitLabel : '单位不一致', officialMedian: officialCosts.length > 0 ? medianScaled(officialCosts) : null }]
  ...
  async function saveCalibration() {
    ...
    await runAction('save-cost-calibration', () => saveCostCalibration({ providerId: profile.id, providerName: profile.name, fundingMode, paidCny, consumableCredit, debitCredit, creditUnitLabel: creditUnit === 'usd' ? '美元额度（USD）' : creditUnit === 'cny' ? '人民币余额（CNY）' : '平台额度', model: benchmarkModel || '未设置模型', probeVersion: 'cost-calibration-v2', costSource: latestProbe?.costCandidate ? latestProbe.costSource ?? 'response_inline' : 'billing_log_manual', probeId: latestProbe?.id, sampleKind: 'cold', officialCny: officialCost ? scaledToDecimal(officialCost) : undefined }))
    setDebitCredit('')
  }

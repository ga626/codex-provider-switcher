...
export function ConnectionDock({
  ...
  const settingState = hasUnsavedChanges ? 'warning' : profile ? 'ok' : 'info'
  const availabilityState = availability.tone === 'success' ? 'ok' : availability.tone
  const renderChecks = (title: string, checks: ValidationCheck[]) => <section className="dock-check-group"><h4>{title}</h4><div className="dock-check-list">{checks.map((check) => { const visual = getCheckVisual(check); return <div className={`dock-check-row ${visual.className}`} key={check.id}>{visual.icon}<div><strong>{check.label}</strong><span>{check.detail}</span>{check.technicalDetail && <details className="dock-technical-detail"><summary>查看技术详情</summary><pre>{check.technicalDetail}</pre></details>}</div></div> })}</div></section>
  return <aside id="connection-dock" className="connection-dock scroll-region" aria-label="连接与切换" data-tour="connection-dock">
    ...
    <dl className="dock-status-list" aria-label="切换准备状态"><div className={environmentReady ? 'ok' : 'warning'}><dt>环境</dt><dd>{environmentReady ? '可用' : '待准备'}</dd></div><div className={settingState}><dt>设置</dt><dd>{hasUnsavedChanges ? '待保存' : profile ? '已保存' : '未选择'}</dd></div><div className={modelReady ? 'ok' : 'warning'}><dt>模型</dt><dd>{modelReady ? providerModelLabel(profile?.model ?? '') : '待选择'}</dd></div><div className={availabilityState}><dt>测试</dt><dd>{availability.shortLabel}</dd></div></dl>

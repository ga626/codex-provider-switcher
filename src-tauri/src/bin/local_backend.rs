...
fn resolve_dist_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    exe_dist.ok_or_else(|| "dist/index.html not found; run npm run build first".into())
}

fn handle_connection(mut stream: TcpStream, dist_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
...
fn read_request(stream: &mut TcpStream) -> Result<(String, String, Vec<u8>), Box<dyn std::error::Error>> {
    ...
    let result = match (method, path.split('?').next().unwrap_or(path)) {
        ...
        ("POST", "/api/profiles/save") => {
            let profile = request_json(body)?
                ...
                .and_then(|value| serde_json::from_value::<EditableProfile>(value).map_err(SwitcherError::from))?;
        ...
        ("POST", "/api/profiles/default") => set_default_profile_core(profile_id(body)?).map(state_json),
        ("POST", "/api/auto-start") => {
...
fn static_asset(dist_dir: &Path, request_path: &str) -> Result<(&'static str, Vec<u8>), Box<dyn std::error::Error>> {
...
fn write_json(stream: &mut TcpStream, status: u16, value: &Value) -> Result<(), Box<dyn std::error::Error>> {

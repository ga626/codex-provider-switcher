...
当你主动执行切换时，应用会读取并更新 Codex 的 `config.toml` 和 `auth.json`，并在写入前创建本地恢复点。为让 Codex 使用 custom provider，`config.toml` 中的 custom provider `api_key` 与 `auth.json` 中所需认证状态会按 Codex 的文件格式保存；它们受当前 Windows 用户账户保护，但不是切换器额外 DPAPI 加密的副本。恢复点、活动记录和 provider 目录保留在本机应用数据目录中。卸载应用不会自动删除这些数据，以便重新安装后恢复；你可以在确认不再需要后自行删除该目录。

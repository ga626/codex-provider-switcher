...
当服务商诊断失败且用户主动打开“报告兼容问题”时，应用只在界面中生成一份脱敏预览：版本、服务商名称、接口域名、模型和失败状态。它不包含访问密钥、配置正文、文件路径、响应原文、Cookie、日志、备份或截图。只有维护者另行部署在线反馈中继、用户阅读预览并勾选同意后，才会向该中继提交上述脱敏内容；当前未配置中继时只能由用户导出内容后自行发送。

...
“运行固定测试”只会在服务商的响应明确带有本次费用时自动填写测试额度。很多 OpenAI-compatible 服务商不会返回这项数据，这不是切换失败；打开该平台的使用日志，复制这次固定测试实际扣除的平台额度，再填写“充值金额”和“平台实际额度”即可保存比较记录。比较时只看同一模型、同一条固定测试的记录；不要把不同模型、不同订阅周期或不同测试长度混在一起。

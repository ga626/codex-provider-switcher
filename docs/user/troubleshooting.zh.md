...
先确认你正在使用的是什么：从 GitHub Release 安装的稳定版，还是开发者打开的源码树开发版。两者的启动和更新方式不同。
...
签名校验失败表示更新包不能被信任，应停止更新并报告问题。

## provider 或模型测试不通过

“刷新模型目录”只说明服务商的 `/v1/models` 可读取，不代表每个模型都能被 Codex 使用。“服务商可用性测试”会用当前模型发送一次短时请求，结果只是一条正向或失败证据，也不能替代你在 Codex 中的实际使用。

- 认证被拒绝：检查 provider 地址和凭据。
- 额度或配额不足：先处理账户余额或配额，再重试。
- 路径、模型或请求不被接受：确认服务商兼容的接口与模型。
- 超时、网络不可达或限流：稍后重试，并检查网络。
- 服务端已响应但应用无法解释：不代表服务商一定不能使用；保留脱敏响应摘要后报告。

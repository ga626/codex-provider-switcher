...
“刷新模型目录”只调用服务商的 `/v1/models`，因此只代表目录可读，不代表模型已经可用于 Codex。运行“兼容性探测”会使用当前模型发送短时、最小的已认证 `/v1/responses` 请求；成功响应带有 Responses `id` 时会为当前模型记录正向证据。

- 探测未确认不会阻止本地安全切换，也不代表当前服务商不能被 Codex 使用。特别是第三方中转可能接受 Codex 的实际请求，却不接受本工具的最小通用探针。
- `接口或模型不可用`、`请求不兼容` 或 `协议不兼容`：确认接口地址的 `/v1` 前缀和模型能力；如 Codex 已正常使用，可将结果视为探针差异而非产品故障。
- `额度不足`、`服务商限流`、`服务端异常`、`请求超时` 或 `网络不可达`：只在需要诊断时按提示处理账户或网络问题，再重试探测。
- 手动输入的模型不在最近目录中时，保存前会要求二次确认；确认和探测都不替代实际 Codex 使用结果。

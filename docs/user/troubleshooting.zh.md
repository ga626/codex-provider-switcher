...
“刷新模型目录”只调用服务商的 `/v1/models`，因此只代表目录可读，不代表模型已经可用于 Codex。运行“真实检查”会使用当前模型发送最小的已认证 `/v1/responses` 请求，且成功响应必须有 Responses `id`。

- `接口或模型不可用`：确认接口地址包含正确的 `/v1` 前缀，并确认当前模型确实支持 Responses。
- `请求不兼容` 或 `协议不兼容`：服务商列出了模型，但没有按 Responses 协议返回可用响应；不要据此直接切换。
- `额度不足`、`服务商限流`、`服务端异常`、`请求超时` 或 `网络不可达`：先按提示处理账户或网络问题，再重新运行真实检查。
- 手动输入的模型不在最近目录中时，保存前会要求二次确认；这不是验证通过，仍需运行真实检查。

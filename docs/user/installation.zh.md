...
GitHub 安装版可在“应用设置 > 更新”中检查签名更新包。更新完成后应用会重新启动；不要手工替换程序目录。
...
新安装的服务商列表为空，不会附带任何其他人的接口地址、模型或 API 密钥。首次打开时，应用只显示“准备连接环境”：阅读知情说明、选择要管理的 Codex 配置层，再点击准备。它会创建首次启动基线并统一 Signalman 所需的 provider/Responses 设置；项目、MCP、插件、hooks、历史和其他未知设置不会被覆盖。多个配置层不会被自动合并或猜测。

准备完成后，点击左侧加号，先选择 ChatGPT 官方账号、厂商官方 API 或中转站。官方账号由 Signalman 发起 Codex 官方登录，账号密码只在 OpenAI 页面输入；官方 API 和中转站分别填写其接口与密钥。在默认模型框内刷新并展开模型目录，目录会在模块内部滚动，并按已验证、需确认和暂不可用分组。

右侧“服务商可用性测试”会优先检查 Responses 完整流式能力，并把基本调用、仅普通聊天、流式中断、认证、额度和网络问题分开显示。“检查并切换”会再次运行测试和写入安全检查；基本调用成功但流式能力未完整证明时也会要求确认风险。确认后才创建恢复点并切换；不要为了测试而手工覆盖 Codex 的 `config.toml` 或 `auth.json`。

最终 provider 切换应在新的 Codex 会话中完成。不要在正在使用的同一会话里切换当前 provider。

## 开机启动

“应用设置”中的“开机后自动打开”默认关闭。只有你主动开启后，应用才会在下次登录 Windows 时自动打开；安装、升级和首次打开都不会启用它。关闭这个开关会移除启动项，关闭应用窗口也不会留下后台常驻进程。

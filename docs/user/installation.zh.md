...
当前已发布的 `0.4.0-alpha` 提供 Windows setup exe、签名更新资产和 fallback zip。本分支准备 `0.5.0-alpha`，修复已验证更新被手动下载页校验错误拦截的问题，并把服务商可用性的判断收束为真实 Responses 请求。桌面安装包是推荐入口；fallback zip 保留给排障、开发和本地 Web 诊断。它不会自动替换旧版工具。
...
恢复完成后，应重新运行目标服务商的真实 Responses 检查；不要把恢复操作或成功刷新模型目录当成 provider 可用性的替代证明。

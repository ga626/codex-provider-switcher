...
`0.4.0-alpha` 发布后，新版桌面应用会提供“交接准备”工作区。它不会自动扫描、覆盖或删除旧目录。

1. 在新版中输入旧 `profiles.json` 文件路径，先执行“预检来源”。
2. 预检只显示可导入条数、格式和冲突数；不会显示 API 密钥。
3. 确认无冲突后执行“导入到新版目录”。旧文件保持只读，新版不会覆盖已有非占位配置。
4. 导入后先逐个完成真实服务商验证。旧工具仍在运行或占用旧端口时，新版会禁止真实切换。
5. 最终停止旧工具、完成一次真实切换和 Codex smoke 必须交给新会话执行，失败则恢复备份并重启旧工具。

完整的交接与回滚顺序见 [旧工具交接手册](../release/legacy-cutover-handoff.zh.md)。

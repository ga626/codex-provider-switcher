...
准备完成后，点击左侧加号添加并保存自己的服务商；在默认模型框内刷新、展开或搜索模型，然后在右侧运行“服务商可用性测试”。测试通过后，“检查并切换”会显示短时变更预览，并在确认时再次检查当前配置没有变化，才会创建恢复点并切换；不要为了测试而手工覆盖 Codex 的 `config.toml` 或 `auth.json`。

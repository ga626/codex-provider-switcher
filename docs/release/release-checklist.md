...
- [ ] 若本版本修复更新器阻断，发布说明写明此版本只是自动更新基线，并把真实应用内升级验收安排到下一个更高版本。
- [ ] 卸载移除程序文件但保留 `%LOCALAPPDATA%\CodeX Provider Switcher`。
- [ ] 当前 Codex 配置摘要脱敏显示。
- [ ] provider 和模型状态能看懂。
- [ ] 模型目录只展示服务商目录和当前模型的正向探测结果，不把 `/models` 读取结果表述为真实可用，也不为所有模型展示“未验证”。
- [ ] 服务商可用性测试由用户显式触发，使用短时、低 token 的已认证 `/v1/responses` 请求；“可调用”“标准 Responses 形状”和“服务端已响应、结果待确认”必须分开显示，未确认不阻止本地安全切换。
- [ ] 目录外手动模型保存前会要求二次确认；确认和探测均不替代实际 Codex 使用结果。
- [ ] 写入前能看到备份/恢复路径。
- [ ] dry-run 不修改真实配置。
- [ ] 最近备份可见，恢复操作要求二次确认。

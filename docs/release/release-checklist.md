...
- [ ] 旧版工具只作为参考源和回滚源。
- [ ] 不承诺自动替换旧工具。
- [ ] 旧 profiles 迁移必须先预检、再显式导入；不得覆盖旧文件或新版已有非占位配置。
- [ ] 旧工具进程和旧端口存在时，新版真实切换必须被阻止。
...
- [ ] 写入前能看到备份/恢复路径。
- [ ] dry-run 不修改真实配置。
- [ ] 旧 profiles 预检只显示脱敏条数、格式和冲突信息，不显示密钥。
- [ ] 备份目录含不记录凭据内容的 manifest。
- [ ] 最终 cutover 交给新会话；旧工具只在该窗口停止，失败时恢复备份并重启旧工具。

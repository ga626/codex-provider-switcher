...
> 日期：2026-07-23
> 状态：实施前事实快照；方案已随 PR #29 合并。随后 `v0.9.0-alpha` 的 GitHub Release 在构建前失败，实际交付状态和恢复步骤以 [发布与交付手册](../maintainers/release-and-delivery.zh.md) 为准。
> 本文保留调研、决定和实施前的基线证据；表中旧安装、旧 workflow、旧目录与“等待合并”描述均是历史事实，不是当前运行要求。

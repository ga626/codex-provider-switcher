...
| 模型目录 | 刷新、搜索、选择模型，并区分目录来源与 Responses 验证状态 |
| 切换面板 | 真实检查、写入、验证、回滚 |
| 安全中心 | 备份、恢复、时间线和配置门禁 |
...
- 不自动把旧模型迁移到新模型。
- 手动输入且不在最近目录中的模型，保存前必须二次确认；目录内模型仍需通过真实 Responses 检查后才能切换。
...
- 所有服务商的真实检查和切换前复验都使用当前模型发送最小的已认证 `POST /v1/responses` 请求，并要求成功响应包含有效 `id`。
- 切换写入前必须同时具备模型名称、已认证 Responses 成功结果和满足配置门禁的状态。
- 探针结果统一归类为配置不完整、鉴权失败、额度不足、限流、接口或模型不可用、请求/协议不兼容、服务端错误、超时或网络/传输失败；HTTP 状态和服务商错误码仅在安全检查诊断中展示。
- 当前模型是否在 provider 返回列表中是警告而非成功证明；目录外模型可以保存，但必须先二次确认并通过真实检查。
...
- 真实检查已覆盖 Responses 协议可用性，但最终 cutover 仍需在独立新会话中按实际目标 provider 和模型完成端到端验证。

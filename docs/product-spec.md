...
- API 密钥保存后不明文显示。
- 编辑时密钥留空表示保留旧密钥。
- 编辑后清除旧的兼容性探测记录；这不会否定用户的实际 Codex 使用结果。
...
- 不自动把旧模型迁移到新模型。
- 手动输入且不在最近目录中的模型，保存前必须二次确认。目录内模型可直接作为切换配置，兼容性探测仅提供补充证据。
...
- 切换不自动发送远端兼容性探测，避免把第三方中转对通用探针的差异误判为不可用。
...
- 兼容性探测由用户显式触发，以当前模型发送最小的已认证 `POST /v1/responses` 请求，成功响应包含有效 `id` 时才为当前模型记录正向证据。
- 探测使用短时预算，且不写入 Codex 配置或凭据；超时、非标准响应、限流或远端错误只表示本次探测未确认，不能推导为服务商不能被 Codex 使用。
- 探测结果可展示鉴权、额度、限流、接口或模型、请求/协议、服务端、超时和网络/传输诊断；它们不是切换门禁。
...
- 兼容性探测只能提供单一最小 Responses 请求的正向证据，不能代替实际 Codex 使用或最终 cutover 的端到端验证。

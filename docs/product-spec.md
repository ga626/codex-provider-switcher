...
状态：`0.4.0-alpha` 候选，已加入旧工具替换前的显式迁移和唯一写入者门禁
...
- 在写入前验证配置和模型选择。
- 写入前自动备份。
- 备份包含不含凭据的 manifest，记录创建时间、原因和文件名。
- 支持恢复和回滚。
...
- 用户显式选择旧 `profiles.json` 后，先只读预检格式、数量和冲突数。
- 只有预检通过才导入；不覆盖新版已有非占位配置，不修改旧文件。
- 导入记录只保存来源文件名、格式、数量和时间，不保存旧文件路径或密钥内容。
- 旧工具进程或旧端口仍存在时，新版禁止真实切换，防止两个工具同时写 Codex 配置。
- 在桌面 GUI 显示迁移、旧进程、旧端口和写入门禁状态。
- 新版试运行成功前不删除旧工具。
- 最终 cutover 生成交接包，由新会话或另一个 agent 执行；当前运行中的 Codex 会话不得执行最终写入。

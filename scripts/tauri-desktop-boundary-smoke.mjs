...
assert(libRs.includes('fn hidden_command(program: &str)'), 'Windows utility calls must use the hidden command helper')
assert(libRs.includes('creation_flags(0x08000000)'), 'Windows utility calls must set CREATE_NO_WINDOW')
assert(libRs.includes('hidden_command("tasklist")'), 'Legacy process detection must use the hidden command helper')

...
for (const requiredProfileId of ['example-provider-a', 'deepseek-official', 'openai-platform', 'chatgpt-oauth', 'hybrid-mixed-auth']) {
  assert(developmentProfileIds.includes(requiredProfileId), `Development demo catalog must include ${requiredProfileId}`)
}
assert(developmentFixtureCatalog.model_catalogs['example-provider-a']?.models?.length >= 4, 'Development demo catalog must include representative Codex model states')
...
assert(appCss.includes('--ranking-columns: 44px minmax(170px, 1.25fr) minmax(124px, .9fr) minmax(128px, .9fr) minmax(130px, .95fr) 72px 52px 66px;'), 'Cost ranking must use one deliberate shared table grid')
...
assert(adapterTs.includes('function isTrustedStoreUrl(value: string)'), 'Store updates must use a dedicated trust check')
...
assert(runtimeRs.includes('event_type == "response.completed"'), 'Provider verification must identify a completed standard Responses stream')
assert(runtimeRs.includes('value.get("id").is_some()'), 'Provider verification must retain the non-stream standard Responses fallback')

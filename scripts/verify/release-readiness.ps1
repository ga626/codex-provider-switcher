param(
    ...
    [ValidateSet("Maintainer", "RunnerSafe")]
    [string]$Mode = "Maintainer",
    [string]$SourceRef = "",
    [switch]$ReportOnly
)
...
function Test-MaintainerRepositoryGovernance {
    param([string]$Repository, [string]$Channel, [string]$ProjectRoot)
    ...
    if ($Channel -in @("github", "all")) {
        ...
        if ($secretExitCode -ne 0) {
            ...
            foreach ($requiredName in @("TAURI_SIGNING_PRIVATE_KEY", "TAURI_SIGNING_PRIVATE_KEY_PASSWORD")) {
    ...
    if ($alerts) {
        foreach ($alert in $alerts) {
            ...
            if ($matchingEntry.Count -gt 0) {
                ...
                if ($reviewBy.Date -ge (Get-Date).Date) { $hasCurrentException = $true }
            }
...
function Get-SourceText {
    ...
    $ErrorActionPreference = "Continue"
    try {
        $result = & git show ("{0}:{1}" -f $SourceRef, $RelativePath) 2>&1
        $exitCode = $LASTEXITCODE
    } finally {
        $ErrorActionPreference = $previousErrorActionPreference
    }
    if ($exitCode -ne 0) {
        Add-Blocker "Unable to read $RelativePath from source ref $SourceRef. $(($result | Out-String).Trim())"
        return ""
    }
...
try {
    foreach ($command in @("git", "gh")) {
        if (-not (Get-Command $command -ErrorAction SilentlyContinue)) { Add-Blocker "Required command is missing: $command" }
    }
    if ($blockers.Count -gt 0) { throw "Required commands are unavailable." }

    if ($Mode -eq "RunnerSafe" -and $Channel -notin @("github", "all")) { Add-Blocker "RunnerSafe mode only supports the GitHub Release channel." }

    $packageText = Get-SourceText -RelativePath "package.json"
    $tauriText = Get-SourceText -RelativePath "src-tauri/tauri.conf.json"
    $cargoText = Get-SourceText -RelativePath "src-tauri/Cargo.toml"
    $package = $packageText | ConvertFrom-Json
    $tauri = $tauriText | ConvertFrom-Json
    $version = [string]$package.version
    if ([string]::IsNullOrWhiteSpace($version)) { Add-Blocker "package.json version is empty." }
    if ($tauri.version -ne $version) { Add-Blocker "package.json and tauri.conf.json versions do not match." }
    if ($cargoText -notmatch ('(?m)^version\s*=\s*"' + [regex]::Escape($version) + '"')) { Add-Blocker "package.json and Cargo.toml versions do not match." }
    if ([string]::IsNullOrWhiteSpace($Tag)) { $Tag = "v$version" }
    if ($Tag -ne "v$version") { Add-Blocker "Requested tag $Tag does not match version $version." }

    if ($Mode -eq "Maintainer") {
        $status = @(& git status --short)
        if (-not $ReportOnly -and $status.Count -gt 0) { Add-Blocker "Working tree is not clean for a formal release." }
        Test-MaintainerRepositoryGovernance -Repository $Repository -Channel $Channel -ProjectRoot $projectRoot
    }
    ...
    if ($Channel -in @("github", "all")) {
        ...
        if ($existingRelease -and $Mode -eq "Maintainer") { Add-Blocker "Release $Tag already exists. Immutable releases must not be overwritten." }
    }
    ...
    Write-Host "Source:     $(if ([string]::IsNullOrWhiteSpace($SourceRef)) { 'working tree' } else { $SourceRef })"
    Write-Host "Enforcement: $(if ($ReportOnly) { 'report-only' } else { 'enforced' })"
    if ($warnings.Count -gt 0) { Write-Host "Warnings:"; foreach ($warning in $warnings) { Write-Host " - $warning" -ForegroundColor Yellow } }
    if ($blockers.Count -gt 0) {
        ...
        if ($ReportOnly) { Write-Host "[REPORT] Code may be merged, but this version is not ready for product delivery." -ForegroundColor Yellow; exit 0 }
        exit 1
    }
